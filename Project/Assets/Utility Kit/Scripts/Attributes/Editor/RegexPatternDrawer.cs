...
/// </summary>
[CustomPropertyDrawer (typeof (RegexPattern))]
public class RegexPatternDrawer : PropertyDrawer 
{
	...
	RegexPattern source { get { return ((RegexPattern)attribute); } }
	...
	void DrawHelpBox (Rect position, SerializedProperty prop) {
		...
		EditorGUI.HelpBox (position, source.helpMessage, MessageType.Error);
	}
	
	// Test if the propertys string value matches the regex pattern.
	bool IsValid (SerializedProperty prop) {
		return Regex.IsMatch (prop.stringValue, source.pattern);
	}
}
